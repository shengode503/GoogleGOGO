# LeetCode Top Interview 150

## Array / String

- [x] Array | Merge Sorted Array | Easy
  - [ ] Review
- [x] Array | Remove Element | Easy
  - [ ] Review
- [x] Array | Remove Duplicates from Sorted Array | Easy
  - [ ] Review
- [x] Array | Remove Duplicates from Sorted Array II | Medium
  - [ ] Review
- [x] Array | Majority Element | Easy
  - [ ] Review
- [x] Array | Rotate Array | Medium
  - [ ] Review
- [x] Array | Best Time to Buy and Sell Stock | Easy
  - [ ] Review
- [x] Array | Best Time to Buy and Sell Stock II | Medium
  - [ ] Review
- [x] Array | Jump Game | Medium
  - [ ] Review
- [ ] Array | Jump Game II | Medium
  - [ ] Review
- [x] Array | H-Index | Medium
  - [ ] Review
- [x] Array | Insert Delete GetRandom O(1) | Medium
  - [ ] Review
- [x] Array | Product of Array Except Self | Medium
  - [ ] Review
- [ ] Array | Gas Station | Medium
  - [ ] Review
- [ ] Array | Candy | Hard
  - [ ] Review
- [ ] Array | Trapping Rain Water | Hard
  - [ ] Review
- [x] String | Roman to Integer | Easy
  - [ ] Review
- [ ] String | Integer to Roman | Medium
  - [ ] Review
- [x] String | Length of Last Word | Easy
  - [ ] Review
- [ ] String | Longest Common Prefix | Easy
  - [ ] Review
- [x] String | Reverse Words in a String | Medium
  - [ ] Review
- [ ] String | Zigzag Conversion | Medium
  - [ ] Review
- [ ] String | Find the Index of the First Occurrence in a String | Easy
  - [ ] Review
- [ ] String | Text Justification | Hard
  - [ ] Review

## Two Pointers

- [ ] Two Pointers | Valid Palindrome | Easy
  - [ ] Review
- [ ] Two Pointers | Is Subsequence | Easy
  - [ ] Review
- [ ] Two Pointers | Two Sum II - Input Array Is Sorted | Medium
  - [ ] Review
- [ ] Two Pointers | Container With Most Water | Medium
  - [ ] Review
- [ ] Two Pointers | 3Sum | Medium
  - [ ] Review

## Sliding Window

- [ ] Sliding Window | Minimum Size Subarray Sum | Medium
  - [ ] Review
- [ ] Sliding Window | Longest Substring Without Repeating Characters | Medium
  - [ ] Review
- [ ] Sliding Window | Substring with Concatenation of All Words | Hard
  - [ ] Review
- [ ] Sliding Window | Minimum Window Substring | Hard
  - [ ] Review

## Matrix

- [ ] Matrix | Valid Sudoku | Medium
  - [ ] Review
- [ ] Matrix | Spiral Matrix | Medium
  - [ ] Review
- [ ] Matrix | Rotate Image | Medium
  - [ ] Review
- [ ] Matrix | Set Matrix Zeroes | Medium
  - [ ] Review
- [ ] Matrix | Game of Life | Medium
  - [ ] Review

## HashMap

- [ ] HashMap | Ransom Note | Easy
  - [ ] Review
- [ ] HashMap | Isomorphic Strings | Easy
  - [ ] Review
- [ ] HashMap | Word Pattern | Easy
  - [ ] Review
- [x] String | Valid Anagram | Easy
  - [x] Review
- [ ] HashMap | Group Anagrams | Medium
  - [ ] Review
- [x] HashMap | Two Sum | Easy
  - [x] Review
- [ ] HashMap | Happy Number | Easy
  - [ ] Review
- [ ] HashMap | Contains Duplicate II | Easy
  - [ ] Review
- [ ] HashMap | Longest Consecutive Sequence | Medium
  - [ ] Review

## Intervals

- [ ] Intervals | Summary Ranges | Easy
  - [ ] Review
- [ ] Intervals | Merge Intervals | Medium
  - [ ] Review
- [ ] Intervals | Insert Interval | Medium
  - [ ] Review
- [ ] Intervals | Minimum Number of Arrows to Burst Balloons | Medium
  - [ ] Review

## Stack

- [ ] Stack | Valid Parentheses | Easy
  - [ ] Review
- [ ] Stack | Simplify Path | Medium
  - [ ] Review
- [ ] Stack | Min Stack | Medium
  - [ ] Review
- [ ] Stack | Evaluate Reverse Polish Notation | Medium
  - [ ] Review
- [ ] Stack | Basic Calculator | Hard
  - [ ] Review

## Linked List

- [ ] Linked List | Linked List Cycle | Easy
  - [ ] Review
- [ ] Linked List | Add Two Numbers | Medium
  - [ ] Review
- [ ] Linked List | Merge Two Sorted Lists | Easy
  - [ ] Review
- [ ] Linked List | Copy List with Random Pointer | Medium
  - [ ] Review
- [ ] Linked List | Reverse Linked List II | Medium
  - [ ] Review
- [ ] Linked List | Reverse Nodes in k-Group | Hard
  - [ ] Review
- [ ] Linked List | Remove Nth Node From End of List | Medium
  - [ ] Review
- [ ] Linked List | Remove Duplicates from Sorted List II | Medium
  - [ ] Review
- [ ] Linked List | Rotate List | Medium
  - [ ] Review
- [ ] Linked List | Partition List | Medium
  - [ ] Review
- [ ] Linked List | LRU Cache | Medium
  - [ ] Review

## Binary Tree General

- [ ] Binary Tree | Maximum Depth of Binary Tree | Easy
  - [ ] Review
- [ ] Binary Tree | Same Tree | Easy
  - [ ] Review
- [ ] Binary Tree | Invert Binary Tree | Easy
  - [ ] Review
- [ ] Binary Tree | Symmetric Tree | Easy
  - [ ] Review
- [ ] Binary Tree | Construct Binary Tree from Preorder and Inorder Traversal | Medium
  - [ ] Review
- [ ] Binary Tree | Construct Binary Tree from Inorder and Postorder Traversal | Medium
  - [ ] Review
- [ ] Binary Tree | Populating Next Right Pointers in Each Node II | Medium
  - [ ] Review
- [ ] Binary Tree | Flatten Binary Tree to Linked List | Medium
  - [ ] Review
- [ ] Binary Tree | Path Sum | Easy
  - [ ] Review
- [ ] Binary Tree | Sum Root to Leaf Numbers | Medium
  - [ ] Review
- [ ] Binary Tree | Binary Tree Maximum Path Sum | Hard
  - [ ] Review
- [ ] Binary Tree | Binary Search Tree Iterator | Medium
  - [ ] Review
- [ ] Binary Tree | Count Complete Tree Nodes | Easy
  - [ ] Review
- [ ] Binary Tree | Lowest Common Ancestor of a Binary Tree | Medium
  - [ ] Review

## Binary Tree BFS

- [ ] Binary Tree BFS | Binary Tree Right Side View | Medium
  - [ ] Review
- [ ] Binary Tree BFS | Average of Levels in Binary Tree | Easy
  - [ ] Review
- [ ] Binary Tree BFS | Binary Tree Level Order Traversal | Medium
  - [ ] Review
- [ ] Binary Tree BFS | Binary Tree Zigzag Level Order Traversal | Medium
  - [ ] Review

## Binary Search Tree

- [ ] BST | Minimum Absolute Difference in BST | Easy
  - [ ] Review
- [ ] BST | Kth Smallest Element in a BST | Medium
  - [ ] Review
- [ ] BST | Validate Binary Search Tree | Medium
  - [ ] Review

## Graph General

- [ ] Graph | Number of Islands | Medium
  - [ ] Review
- [ ] Graph | Surrounded Regions | Medium
  - [ ] Review
- [ ] Graph | Clone Graph | Medium
  - [ ] Review
- [ ] Graph | Evaluate Division | Medium
  - [ ] Review
- [ ] Graph | Course Schedule | Medium
  - [ ] Review
- [ ] Graph | Course Schedule II | Medium
  - [ ] Review

## Graph BFS

- [ ] Graph BFS | Snakes and Ladders | Medium
  - [ ] Review
- [ ] Graph BFS | Minimum Genetic Mutation | Medium
  - [ ] Review
- [ ] Graph BFS | Word Ladder | Hard
  - [ ] Review

## Trie

- [ ] Trie | Implement Trie (Prefix Tree) | Medium
  - [ ] Review
- [ ] Trie | Design Add and Search Words Data Structure | Medium
  - [ ] Review
- [ ] Trie | Word Search II | Hard
  - [ ] Review

## Backtracking

- [ ] Backtracking | Letter Combinations of a Phone Number | Medium
  - [ ] Review
- [ ] Backtracking | Combinations | Medium
  - [ ] Review
- [ ] Backtracking | Permutations | Medium
  - [ ] Review
- [ ] Backtracking | Combination Sum | Medium
  - [ ] Review
- [ ] Backtracking | N-Queens II | Hard
  - [ ] Review
- [ ] Backtracking | Generate Parentheses | Medium
  - [ ] Review
- [ ] Backtracking | Word Search | Medium
  - [ ] Review

## Divide & Conquer

- [ ] Divide & Conquer | Convert Sorted Array to Binary Search Tree | Easy
  - [ ] Review
- [ ] Divide & Conquer | Sort List | Medium
  - [ ] Review
- [ ] Divide & Conquer | Construct Quad Tree | Medium
  - [ ] Review
- [ ] Divide & Conquer | Merge k Sorted Lists | Hard
  - [ ] Review

## Kadane's Algorithm

- [ ] Kadane | Maximum Subarray | Medium
  - [ ] Review
- [ ] Kadane | Maximum Sum Circular Subarray | Medium
  - [ ] Review

## Binary Search

- [ ] Binary Search | Search Insert Position | Easy
  - [ ] Review
- [ ] Binary Search | Search a 2D Matrix | Medium
  - [ ] Review
- [ ] Binary Search | Find Peak Element | Medium
  - [ ] Review
- [ ] Binary Search | Search in Rotated Sorted Array | Medium
  - [ ] Review
- [ ] Binary Search | Find First and Last Position of Element in Sorted Array | Medium
  - [ ] Review
- [ ] Binary Search | Find Minimum in Rotated Sorted Array | Medium
  - [ ] Review
- [ ] Binary Search | Median of Two Sorted Arrays | Hard
  - [ ] Review

## Heap

- [ ] Heap | Kth Largest Element in an Array | Medium
  - [ ] Review
- [ ] Heap | IPO | Hard
  - [ ] Review
- [ ] Heap | Find K Pairs with Smallest Sums | Medium
  - [ ] Review
- [ ] Heap | Find Median from Data Stream | Hard
  - [ ] Review

## Bit Manipulation

- [ ] Bit Manipulation | Add Binary | Easy
  - [ ] Review
- [ ] Bit Manipulation | Reverse Bits | Easy
  - [ ] Review
- [ ] Bit Manipulation | Number of 1 Bits | Easy
  - [ ] Review
- [ ] Bit Manipulation | Single Number | Easy
  - [ ] Review
- [ ] Bit Manipulation | Single Number II | Medium
  - [ ] Review
- [ ] Bit Manipulation | Bitwise AND of Numbers Range | Medium
  - [ ] Review

## Math

- [ ] Math | Palindrome Number | Easy
  - [ ] Review
- [ ] Math | Plus One | Easy
  - [ ] Review
- [ ] Math | Factorial Trailing Zeroes | Medium
  - [ ] Review
- [ ] Math | Sqrt(x) | Easy
  - [ ] Review
- [ ] Math | Pow(x, n) | Medium
  - [ ] Review
- [ ] Math | Max Points on a Line | Hard
  - [ ] Review

## 1D DP

- [ ] DP | Climbing Stairs | Easy
  - [ ] Review
- [ ] DP | House Robber | Medium
  - [ ] Review
- [ ] DP | Word Break | Medium
  - [ ] Review
- [ ] DP | Coin Change | Medium
  - [ ] Review
- [ ] DP | Longest Increasing Subsequence | Medium
  - [ ] Review

## Multidimensional DP

- [ ] DP | Triangle | Medium
  - [ ] Review
- [ ] DP | Minimum Path Sum | Medium
  - [ ] Review
- [ ] DP | Unique Paths II | Medium
  - [ ] Review
- [ ] DP | Longest Palindromic Subsequence | Medium
  - [ ] Review
- [ ] DP | Interleaving String | Medium
  - [ ] Review
- [ ] DP | Edit Distance | Medium
  - [ ] Review
- [ ] DP | Best Time to Buy and Sell Stock III | Hard
  - [ ] Review
- [ ] DP | Best Time to Buy and Sell Stock IV | Hard
  - [ ] Review
- [ ] DP | Maximal Square | Medium
  - [ ] Review
